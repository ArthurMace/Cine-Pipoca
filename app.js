    ...
    try {
        ...
        if (dados.results && dados.results.length > 0) {
            ...
            return {
                ...
                sinopse: item.overview || "Sem sinopse.",
...
window.adicionar = async function() {
    ...
    if (!id && !imagem && nome) {
        ...
        if (dadosTMDB) {
            ...
            extrasAuto = { sinopse: dadosTMDB.sinopse, banner: dadosTMDB.banner, totalSeasons: dadosTMDB.totalSeasons, totalEpisodes: dadosTMDB.totalEpisodes };
        }
    }
...
window.salvarNotaIndividual = async function() {
    ...
    if (perfilAtivo === 'arthur') {
        item.notaArthur = document.getElementById("aval-nota").value;
        item.comentarioArthur = document.getElementById("aval-comentario").value;
    } else {
        item.notaDay = document.getElementById("aval-nota").value;
        item.comentarioDay = document.getElementById("aval-comentario").value;
    }
    if (item.notaArthur && item.notaDay) {
        item.status = 'assistido';
        item.comentario = `A: ${item.comentarioArthur || ""} | D: ${item.comentarioDay || ""}`;
    } else { item.status = 'avaliacao'; }
...
window.sortearFilme = function() {
    ...
    if (opcoes.length === 0) return alert("Nenhum filme disponível!");
    window.verSinopse(opcoes[Math.floor(Math.random() * opcoes.length)].firebaseId);
};
    ...
    document.getElementById("container-sorteado").innerHTML = `
        <div style="background: url('${item.banner || ''}') center/cover; border-radius:15px; overflow:hidden;">
            <div style="background:rgba(15, 23, 42, 0.95); padding:30px; backdrop-filter:blur(5px);">
                ...
                <p style="text-align:left; font-size:14px; line-height:1.6;">${item.sinopse || "Sem sinopse."}</p>
...
window.excluirItem = async (id) => {
    if (confirm("Excluir?")) { await deleteItem(id); data = await getData(); window.render(); }
};
    ...
    return lista.map(item => {
        ...
        <div class="card ${jaAssistido ? 'card-finalizado' : ''}" onclick="window.verSinopse('${item.firebaseId}')">
            ...
            <div class="info">
                ...
                ${!jaAssistido && !votou ? `<button class="btn-primary" style="font-size:10px; padding:5px; margin-top:10px;" onclick="event.stopPropagation(); ${item.status === 'quero' ? `window.abrirModal('${item.firebaseId}')` : `window.finalizarRapido('${item.firebaseId}')` }">${item.status === 'quero' ? 'Assistir' : 'Nota'}</button>` : ''}
                ${jaAssistido ? `<p style="margin-top:10px; font-size:10px;">🤵‍♂️ ${'🍿'.repeat(item.notaArthur || 0)} | 👰‍♀️ ${'🍿'.repeat(item.notaDay || 0)}</p>` : ''}
                <button onclick="event.stopPropagation(); window.excluirItem('${item.firebaseId}')" style="background:rgba(239, 68, 68, 0.2); color:#ef4444; border:none; padding:4px 8px; border-radius:5px; font-size:9px; margin-top:10px;">Excluir</button>
            </div>
        </div>`;
    }).join("");
}
...
window.render = function() {
    ...
    const contHome = document.getElementById("home");
    if (paginaAtual === "home" && contHome) {
        contHome.innerHTML = `
    ...
    if (paginaAtual === "series" && contSeries) contSeries.innerHTML = renderCards(filtrados.filter(i => i.tipo === "serie"));

    const contFilmes = document.getElementById("filmes");
    if (paginaAtual === "filmes" && contFilmes) contFilmes.innerHTML = renderCards(filtrados.filter(i => i.tipo === "filme"));

    const contQuero = document.getElementById("queroList");
    if (paginaAtual === "quero" && contQuero) contQuero.innerHTML = renderCards(filtrados.filter(i => i.status === "quero"));
};
